...
The point would be to reuse as many parts from Capstone Week 3 as possible. I used data from my Alpaca.markets subscription when running locally (using my keys) or publicly on csv file hosted on GitHub. However, with only ~1,245 days of SPY data and 128 test sequences, the model was not able to learn much.
...
The model generates text samples using temperature-controlled sampling after training on ~30,000 characters.
...
The attention mechanism enables the model to identify which past days are most relevant for current volatility.
Unlike GARCH/EWMA which use fixed decay weights, transformers can learn dynamic, content-based dependencies.
